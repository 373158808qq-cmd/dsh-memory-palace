...
export function registerInitRoutes(ctx, deps) {
  ...
  R('/dsh-memory-app/init/status', async function () { return await initStatus(ctx, deps) })

  R('/dsh-memory-app/init/ollama/locate', async function (req) {
  ...
  R('/dsh-memory-app/init/ollama/install', async function () { return installOllama() })

  R('/dsh-memory-app/init/skeleton/plan', async function (req) {
    const b = await readBody(req)
    return { ok: true, plan: skeletonPlan(b.root) }
  })
  R('/dsh-memory-app/init/skeleton/create', async function (req) {
    const b = await readBody(req)
    if (!b.root) return { ok: false, error: '需要先选工作区根目录' }
    return Object.assign({ ok: true }, createSkeleton(b.root, b.picks))
  })

  R('/dsh-memory-app/init/sop/analyze', async function (req) {
    const b = await readBody(req)
    const r = await analyzeDocForSop(ctx, b.path, { text: b.text, name: b.name })
    return Object.assign({ ok: true }, r)
  })
  R('/dsh-memory-app/init/sop/save', async function (req) {
    const b = await readBody(req)
  ...
  R('/dsh-memory-app/init/sop/default', async function () {
  ...
  R('/dsh-memory-app/init/pick', async function (req) {
    const b = await readBody(req)
  ...
  ctx.webServer.register({ kind: 'exact', path: '/dsh-memory-app/init/ls', handler: async function (req, res) {
    try {
  ...
  R('/dsh-memory-app/init/pick/result', async function () {
    const f = pickFile()
  ...
  R('/dsh-memory-app/init/task', async function (req) {
    const b = await readBody(req)
  ...
  R('/dsh-memory-app/init/session', async function (req) {
    const b = await readBody(req)
  ...
  R('/dsh-memory-app/init/check', async function () { return await initStatus(ctx, deps) })
  return ctx
}
