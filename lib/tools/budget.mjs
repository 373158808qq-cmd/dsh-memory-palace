...
//     · `dsh-memory-app`（我们的插件）按**字符数**卡：AGENTS.md / PROJECT_LEDGER.md / inject.json
...
const APP = join(ROOT, 'plugins', 'dsh-memory-app', 'lib', 'index.js')         // 我们的压缩守卫
...
const SOURCES = [
  ...
  { label: 'AGENTS.md（当前目录）', by: 'dsh-memory-app', path: join(ROOT, 'AGENTS.md'), unit: 'char', cap: 4000, src: APP + ':1082' },
  { label: 'PROJECT_LEDGER.md（当前目录）', by: 'dsh-memory-app', path: join(ROOT, 'PROJECT_LEDGER.md'), unit: 'char', cap: 8000, src: APP + ':1083' },
  { label: '★SOP.md（协作 SOP）', by: 'dsh-memory-app', path: join(MEM, 'SOP.md'), unit: 'char', cap: 8000, src: APP + ':1048 SOP_CAP', manual: ['memory/AGENTS.md', 'memory/PROJECT_LEDGER.md', 'memory/INDEX.md'] },
...
function verifyCaps() {
  ...
  const sopAt = appSrc.indexOf("name: 'dsh-memory-app-sop'")
...
for (const f of extraInjected()) {
  ...
  rows.push({ label: '额外注入：' + f.split(/[\\/]/).pop(), by: 'dsh-memory-app (inject.json)', path: abs, unit: 'char', cap: 8000, src: APP + ':1104' })
}
...
//   `~/.dsh/profiles/web/node_modules/dsh-memory-app/`，`<工作区>/plugins/` 根本不存在
...
let vFail = 0
if (!SRC_PRESENT) {
  console.log('  · 跳过：这台机器上 `<工作区>/plugins/dsh-memory-app/lib/index.js` 不存在（插件是装在 profile 里的，正常）')
