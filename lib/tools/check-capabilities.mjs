...
//                                     加 `--tools` 才去问插件的 `/dsh-memory-app/tools` 路由 ——
...
//   node memory/tools/check_capabilities.mjs --tools --tools-url http://127.0.0.1:3081/dsh-memory-app/tools
...
//   加 `--tools` 才去问插件的 `/dsh-memory-app/tools` 路由（它读的是**真注册表**，不是我们写死的名单）。
// ★三条不许违反★：
...
const TOOLS_URL = argAfter('--tools-url',
  process.env.DSH_MEMORY_TOOLS_URL || 'http://127.0.0.1:3080/dsh-memory-app/tools')
...
/** 找根：从本文件往上找工作区根（标志物 = plugins/dsh-memory-app；不写死本机路径） */
function findRoot() {
  let dir = HERE
  for (let i = 0; i < 8; i++) {
    if (existsSync(join(dir, 'plugins', 'dsh-memory-app', 'lib', 'vault-sync.mjs'))) return dir
