...
export const name = 'dsh-memory-palace'
...
/** GET /dsh-memory-palace/corpus/status —— 只读：开关状态 + 待转写清单 + 最近触发记录 */
...
/** POST /dsh-memory-palace/corpus/run —— 手动补转写。默认最多 5 份；带 `{"all":true}` 则不限 */
...
async function bootCleanup() {
  ...
  if (runners.killed.length > 0) {
    console.log('[dsh-memory-palace] 启动清理：收掉了 ' + runners.killed.length +
...
async function doVaultSync(trigger) {
  ...
  try {
    ...
    if (!existsSync(join(lastKnownCwd, 'memory'))) {
      ...
      if (!v.ok) console.log('★[dsh-memory-palace] 记忆视图刷新失败★（库没事，显示层会停在旧版）：' + v.error)
    } catch (e) {
      r.view = { ok: false, error: String((e && e.message) || e) }
      console.log('★[dsh-memory-palace] 记忆视图刷新抛错★：' + ((e && e.message) || e))
    }
    lastSyncResult = r
    return r
  } catch (e) {
...
/** GET /dsh-memory-palace/vault-sync/status —— 只读检查，给前端显示"过期了没" */
...
/** POST /dsh-memory-palace/vault-sync/run —— 手动一键重建 */
...
/** GET /dsh-memory-palace/view —— 只读状态（开没开、库在哪、登记了没、上次动作） */
...
/** POST /dsh-memory-palace/view/open —— ① 先刷视图（用户拍板：打开前刷新）② 确保库登记 ③ 打开 */
...
/** POST /dsh-memory-palace/view/close —— 先优雅关；带 {force:true} 才强杀 */
...
export function apply(ctx) {
  ...
  if (!globalThis.__dshMemoryPersistMounted) {
    ...
    import('./vendor/dsh-persist/index.js').then(function (persist) {
      ...
      console.error('★[dsh-memory-palace] 挂载内置分叉失败（USER/MEMORY 注入与 memory 工具会缺失）★', e)
    })
  }
  ...
  ctx.webServer.register({ kind: 'exact', path: '/dsh-memory-palace/sop', handler: sopHandler })
  ...
  ctx.webServer.register({ kind: 'exact', path: '/dsh-memory-palace/embedder', handler: embedderHandler })
  ctx.webServer.register({ kind: 'exact', path: '/dsh-memory-palace/embedder/selftest', handler: embedderSelftestHandler })
  // Ollama 进程：检测在状态里，这里给启停、拉取模型、预热、卸载的出口
  ctx.webServer.register({ kind: 'exact', path: '/dsh-memory-palace/ollama/start', handler: ollamaActionHandler })
  ctx.webServer.register({ kind: 'exact', path: '/dsh-memory-palace/ollama/stop', handler: ollamaActionHandler })
  ctx.webServer.register({ kind: 'exact', path: '/dsh-memory-palace/ollama/pull', handler: ollamaActionHandler })
  ctx.webServer.register({ kind: 'exact', path: '/dsh-memory-palace/ollama/warm', handler: ollamaActionHandler })
  ctx.webServer.register({ kind: 'exact', path: '/dsh-memory-palace/ollama/unload', handler: ollamaActionHandler })
  // 收掉残留的本地模型进程（服务没了但显存还被占着时的出口）
  ctx.webServer.register({ kind: 'exact', path: '/dsh-memory-palace/ollama/clean', handler: ollamaActionHandler })
  // 写入流：把核心记忆（或任意长文）按结构切块下沉进 Vault
  ctx.webServer.register({ kind: 'exact', path: '/dsh-memory-palace/ingest', handler: ingestHandler })
  // Vault 同步：状态（只读检查，显示"过期了没"）+ 手动一键重建
  ctx.webServer.register({ kind: 'exact', path: '/dsh-memory-palace/vault-sync/status', handler: vaultSyncStatusHandler })
  ctx.webServer.register({ kind: 'exact', path: '/dsh-memory-palace/vault-sync/run', handler: vaultSyncRunHandler })
  // ⑨ 自动转写：状态（只读）+ 手动补一把
  ctx.webServer.register({ kind: 'exact', path: '/dsh-memory-palace/corpus/status', handler: corpusStatusHandler })
  ctx.webServer.register({ kind: 'exact', path: '/dsh-memory-palace/corpus/run', handler: corpusRunHandler })
  // ⑤ 语义判官：**只读**状态路由（"上次跑成什么样"）。
  //   ★跑的那一下只在下面那个工具里★ —— 这个路由绝不触发外发（它是以后设置页按钮的读数口）。
  ctx.webServer.register({
    kind: 'exact',
    path: '/dsh-memory-palace/semantic/status',
    handler: function (req, res) { return semanticStatusHandler(req, res, ctx) }
  })
  // ⑧ 记忆视图（Obsidian）：状态（只读）/ 打开（先刷新再开）/ 关闭（先优雅，带 {force:true} 才强杀）
  ctx.webServer.register({ kind: 'exact', path: '/dsh-memory-palace/view', handler: viewStatusHandler })
  ctx.webServer.register({ kind: 'exact', path: '/dsh-memory-palace/view/open', handler: viewOpenHandler })
  ctx.webServer.register({ kind: 'exact', path: '/dsh-memory-palace/view/close', handler: viewCloseHandler })
  // 工具注册表（**只读**）：回答"这个进程现在注册了哪些工具" —— 给能力点名器做 `tool:` 的运行时核。
  // ★它是只读的、且**不碰任何用户的东西**★（只问注册表），所以测试可以放心在真服务端上打它。
  ctx.webServer.register({
    kind: 'exact',
    path: '/dsh-memory-palace/tools',
  ...
  ctx.systemPrompt.section({
    name: 'dsh-memory-palace-discipline',
    order: 58,
